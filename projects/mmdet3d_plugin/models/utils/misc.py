...
def transform_reference_points(reference_points, egopose, l2e_rotation, reverse=False, translation=True):
    reference_points_center = torch.cat([reference_points[...,:3], torch.ones_like(reference_points[..., 0:1])], dim=-1) # (8, 1280, 4)
    reference_points_rot = reference_points[..., 6:8]  # (8, 1280, 2)
    reference_points_vel = reference_points[..., 8:]

    reference_points[...,:3] = (egopose.unsqueeze(1) @ reference_points_center.unsqueeze(-1)).squeeze(-1)[..., :3]

    ref_rotation_mat = torch.zeros([reference_points_rot.shape[0], reference_points_rot.shape[1], 2, 2]).cuda()
    ref_rotation_mat[:, :, 0, 0] = reference_points_rot[:, :, 1]
    ref_rotation_mat[:, :, 0, 1] = -reference_points_rot[:, :, 0]
    ref_rotation_mat[:, :, 1, 0] = reference_points_rot[:, :, 0]
    ref_rotation_mat[:, :, 1, 1] = reference_points_rot[:, :, 1]

    if reverse: # pre-update
        ref_rotation_mat = l2e_rotation[..., None, :2, :2] @ egopose[..., None, :2, :2] @ ref_rotation_mat
        reference_points_vel = l2e_rotation[..., None, :2, :2] @ egopose[..., None, :2, :2] @ reference_points_vel.unsqueeze(-1)

    else:
        ref_rotation_mat = egopose[..., None, :2, :2] @ l2e_rotation[..., None, :2, :2] @ ref_rotation_mat
        reference_points_vel = egopose[..., None, :2, :2] @ l2e_rotation[..., None, :2, :2] @ reference_points_vel.unsqueeze(-1)
    reference_points[..., 6:7] = -ref_rotation_mat[..., 0, 1].unsqueeze(-1)
    reference_points[..., 7:8] = ref_rotation_mat[..., 0, 0].unsqueeze(-1)
    reference_points[..., 8:10] = reference_points_vel.squeeze(-1)
