...
def get_global_pos(points, norm_range):
    pc_range, scale_range, yaw_range, vel_range = norm_range
    points_center = points[..., 0:3] * (pc_range[3:6] - pc_range[0:3]) + pc_range[0:3]
    points_scale = points[..., 3:6] * (scale_range[3:6].log() - scale_range[0:3].log()) + scale_range[0:3].log()
    points_rot = points[..., 6:8] * (yaw_range[2:4] - yaw_range[0:2]) + yaw_range[0:2]

    points = torch.cat([points_center, points_scale, points_rot], dim=-1)
    return points
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            if lid > 0:
                ...
                query_reference_embed = query_center_embed + query_scale_embed + query_rot_embed + query_vel_embed
                query_pos = self.query_embedding(query_reference_embed)
            ...
            if self.reg_branches is not None:
                tmp = self.reg_branches[lid](query)
                tmp[..., :10] += inverse_sigmoid(reference_points)

                new_reference_points = tmp[..., :10].sigmoid()
        ...
        return torch.stack(intermediate), torch.stack(inter_references)
...
class DeformableFeatureAggregationCuda(BaseModule):
    def __init__(
        ...
        # vertex location
        self.vertex_point = nn.Parameter(
            torch.cat([torch.tensor([0, 0, 0]).unsqueeze(0),
            torch.tensor([0.45, 0.45, 0.45]).unsqueeze(0),
            torch.tensor([0.45, 0.45, -0.45]).unsqueeze(0),
            torch.tensor([0.45, -0.45, 0.45]).unsqueeze(0),
            torch.tensor([-0.45, 0.45, 0.45]).unsqueeze(0),
            torch.tensor([0.45, -0.45, -0.45]).unsqueeze(0),
            torch.tensor([-0.45, -0.45, 0.45]).unsqueeze(0),
            torch.tensor([-0.45, 0.45, -0.45]).unsqueeze(0),
            torch.tensor([-0.45, -0.45, -0.45]).unsqueeze(0)], dim=0), requires_grad=False)
        vertex_pts = self.vertex_point.shape[0]

        self.weights_fc = nn.Linear(self.embed_dims, self.num_groups * self.num_levels * (num_pts + vertex_pts) * 2)
    ...
    def forward(self, instance_feature, query_pos, feat_flatten, reference_points, spatial_flatten, level_start_index, norm_range, lidar2img_mat, img_metas):
        bs, num_anchor = reference_points.shape[:2]
        reference_points = get_global_pos(reference_points, norm_range)

        scale = (
                self.learnable_fc(instance_feature)
                .reshape(bs, num_anchor, self.num_pts, 3)
                .sigmoid()
                - 0.5
        )

        fix_scale = self.fix_scale.detach()
        fix_scale = fix_scale.expand(bs, num_anchor, -1, -1)
        scale = torch.cat([scale, fix_scale], dim=-2)

        vertex_pos = scale * reference_points[..., None, 3:6].exp()
        # distance-wise feature sampling
        key_points_far = reference_points[..., :3].unsqueeze(-2) + vertex_pos / (torch.sum(reference_points[..., 0:3] ** 2, dim=-1).unsqueeze(-1).unsqueeze(-1) ** (1/2)).log()
        key_points_near = reference_points[..., :3].unsqueeze(-2) + vertex_pos
        key_points = torch.cat([key_points_far, key_points_near], dim=2).reshape(bs, num_anchor, -1, 3)
    ...
    def _get_weights(self, instance_feature, anchor_embed, lidar2img_mat):
        ...
        cam_embed = self.cam_embed(lidar2img)
