...
class YOLOXHeadCustom(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.multi_level_pred = multi_level_pred
    ...
    def get_bboxes(self, preds_dicts,
        ...
        mlvl_priors = self.prior_generator.grid_priors(featmap_sizes, dtype=cls_scores[0].dtype,
                                                       device=cls_scores[0].device, with_stride=True)  # 3x(Hi*Wi, 4)
        B = num_imgs // 6
        ...
        # extract valid data based on objectnesses
        ...
        tmp_sample_weight = flatten_sample_weight.clone().reshape(B, -1, 1)
        tmp_valid_indices = valid_indices.clone().reshape(B, 6, -1, 1)
        max_k_value = -1
        for b in range(B):
            k_value = tmp_valid_indices[b].sum()
            if k_value > max_k_value:
                max_k_value = k_value
        if max_k_value > 0:
            for b in range(B):
                _, k_index = torch.topk(tmp_sample_weight[b], k=max_k_value, dim=0)
                flatten_img_index = b*6+k_index.squeeze()//valid_indices.shape[1]
                flatten_token_index = k_index.squeeze()%valid_indices.shape[1]
                valid_indices[flatten_img_index, flatten_token_index, 0] = True
